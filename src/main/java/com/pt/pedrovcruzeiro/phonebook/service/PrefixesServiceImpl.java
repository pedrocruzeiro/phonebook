...
public class PrefixesServiceImpl implements PrefixesService {
  ...
  public void loadClientPrefixesFile() {
    ...
    try {
      ...
      log.error(
          PhonebookLogFormatter.format("Failed to load prefixes file :: )" + e.getMessage() + "."));
      log.debug(PhonebookLogFormatter.format("Loading default prefixes file."));
      try {
        InputStream prefixesInputStream =
            fileUtils.retrieveInputStream(defaultPrefixesFilePathName);
        fileUtils.addFileLinesToStringSet(prefixesInputStream, filePrefixes);
      } catch (IOException ioException) {
        log.error(
            PhonebookLogFormatter.format(
                "Failed to load default prefixes file :: )" + e.getMessage() + "."));
      }
    }
  }
}
